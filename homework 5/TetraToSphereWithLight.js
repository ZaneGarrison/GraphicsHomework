 ...
 * Assignment #: Mod 5 Assignment 2 part 1
 */
...
window.onload = function init() {
	...
	document.getElementById("increaseSubdivisions").onclick =
      function () {
          if(numTimesToSubdivide<7){  numTimesToSubdivide += 1;}
		index = 0;
		pointsArray = [];
		normalsArray = [];
      ...
      function (event) {
	if (event.keyCode == 65 || event.keyCode == 37) {   // Left Arrow
	theta += 0.1;
          }
    if (event.keyCode == 68 || event.keyCode == 39) {   // Right Arrow
	theta -= 0.1;
          }
    if (event.keyCode == 87 || event.keyCode == 38) {   // Up Arrow
	phi -= 0.1;
          }
    if (event.keyCode == 83 || event.keyCode == 40) {   // Down Arrow
	phi += 0.1;
          }
	  },false);
